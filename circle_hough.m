...
%size(featInd)
%size(tempInd)
% templates=repmat(tempInd,length(featInd),1);
% %size(templates)
% templates=templates+repmat(featInd',1,length(tempInd));
% %size(templates)
% %size(templates(:))
% class(templates)
% h_one_time=accumarray(templates(:),1);
% [h_histo,bins]=histo(templates(:));
% 'h_histo'
% size(h_histo)
% size(h_one_time)
% size(bins)
...
% size(h_one_time)
% size(h)
% isequal(h_one_time,h(1:length(h_one_time)))
% 
